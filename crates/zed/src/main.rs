...
fn init_paths() -> HashMap<io::ErrorKind, Vec<&'static Path>> {
    // First, ensure instance-specific directories exist
    if let Err(e) = utils::ensure_directories() {
        eprintln!("Failed to create instance directories: {}", e);
    }
    
    // Then create the application-specific directories
    [
        paths::config_dir(),
