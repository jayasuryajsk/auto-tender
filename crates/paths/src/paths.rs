//! Paths to locations used by the editor.
...
/// On macOS, this is `~/Library/Application Support/Editor-{instance_id}`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/editor-{instance_id}`.
/// On Windows, this is `%LOCALAPPDATA%\Editor-{instance_id}`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/editor-{instance_id}`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/editor-{instance_id}`.
/// On Windows, this is `%APPDATA%\Editor-{instance_id}`.
static CONFIG_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Returns the relative path to the server directory on the ssh host.
pub fn remote_server_dir_relative() -> &'static Path {
    static REMOTE_SERVER_DIR: OnceLock<PathBuf> = OnceLock::new();
    REMOTE_SERVER_DIR.get_or_init(|| PathBuf::from(format!(".editor_server-{}", instance::instance_id()))).as_path()
}
...
/// Returns the path to the configuration directory used by the editor.
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.join("config")
        } else {
            // Use the instance-specific configuration directory
            instance::config_dir()
        }
    })
}

/// Returns the path to the data directory used by the editor.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
        } else {
            // Use the instance-specific data directory
            instance::data_dir()
        }
    })
}
/// Returns the path to the temp directory used by the editor.
pub fn temp_dir() -> &'static PathBuf {
    static TEMP_DIR: OnceLock<PathBuf> = OnceLock::new();
    TEMP_DIR.get_or_init(|| {
        // Use the instance-specific cache directory
        instance::cache_dir()
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs").join(format!("Editor-{}", instance::instance_id()))
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the remote server directory on this SSH host.
pub fn remote_server_state_dir() -> &'static PathBuf {
    static REMOTE_SERVER_STATE: OnceLock<PathBuf> = OnceLock::new();
    REMOTE_SERVER_STATE.get_or_init(|| data_dir().join(format!("server_state-{}", instance::instance_id())))
}

/// Returns the path to the log file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("Editor.log"))
}

/// Returns the path to the old log file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("Editor.log.old"))
}
...
pub fn crashes_dir() -> &'static Option<PathBuf> {
    ...
    CRASHES_DIR.get_or_init(|| {
        // Add instance ID to crash paths to separate instances
...
/// Returns the relative path to a settings folder within a project.
